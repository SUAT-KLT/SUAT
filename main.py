...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:1337@localhost/atu1'
...
if __name__ == '__main__':
    ...
    ports_to_try = [5000, 8080, 8000, 8888, 5001]

    for port in ports_to_try:
        try:
            print(f"\n🚀 Пробуем запустить на порту {port}")
            ...
            print(f"🌍 Для доступа из интернета: http://ВАШ-ВНЕШНИЙ-IP:{port}")
            print("=" * 50)
            ...
            print(f"❌ Порт {port} не доступен: {e}")
            continue